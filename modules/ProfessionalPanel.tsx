...
import { Calendar, User, DollarSign, LogOut, Clock, Ban, ShoppingBag, Plus, CalendarRange, Settings } from 'lucide-react';
...
export const ProfessionalPanel: React.FC<{
    ...
    const [activeTab, setActiveTab] = useState<'agenda' | 'performance' | 'store' | 'settings' | 'profile'>('agenda');
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'store':
                return (
                    <div className="space-y-4">
                        <h3 className="font-bold text-gray-900">Produtos / Vendas</h3>
                        <div className="bg-orange-50 p-4 rounded-xl text-center text-orange-800 text-sm">
                            Funcionalidade de vendas rápidas em breve!
                            <br />
                            Aqui você poderá lançar produtos vendidos para os clientes.
                        </div>
                    </div>
                );
            case 'profile':
                return (
                    <div className="space-y-4">
                        <div className="flex flex-col items-center py-6 bg-white rounded-xl border border-gray-100 shadow-sm">
                            <img src={professional.avatarUrl} className="w-24 h-24 rounded-full border-4 border-white shadow-lg mb-3" />
                            <h2 className="text-xl font-bold text-gray-900">{professional.name}</h2>
                            <p className="text-gray-500 text-sm">{professional.email}</p>
                        </div>

                        <Card title="Suas Taxas">
                            <div className="space-y-3">
                                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                    <span className="text-gray-600">Comissão Serviços</span>
                                    <span className="font-bold text-gray-900">{professional.commissionRate}%</span>
                                </div>
                                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                    <span className="text-gray-600">Comissão Produtos</span>
                                    <span className="font-bold text-gray-900">{professional.productCommissionRate || 0}%</span>
                                </div>
                            </div>
                        </Card>
                    </div>
        ...
        <AppShell
            ...
            bottomNav={
                <MobileNav>
                    ...
                    <MobileNavItem icon={<ShoppingBag />} label="Sacolinha" active={activeTab === 'store'} onClick={() => setActiveTab('store')} />
                    <MobileNavItem icon={<DollarSign />} label="Comissões" active={activeTab === 'performance'} onClick={() => setActiveTab('performance')} />
                    <MobileNavItem icon={<User />} label="Perfil" active={activeTab === 'profile'} onClick={() => setActiveTab('profile')} />
                    <MobileNavItem icon={<Settings />} label="Ajuste" active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} />
                </MobileNav>
            }
        >
            <div className="p-4">
